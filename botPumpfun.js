 ...
 *********************/
const CONFIG = {
    API_KEY: 'your-api-c91n2mtrdt436auq5dgncwbj85w5ehbn94r5eeb26grp8wuqehcnjc318x532p2p9t84cpaab4v42ka4d9p64uj75x3k0pkh6ta7apba8d2mpbv79n3p6ra9ed37evjrdt9n2kb3cwyku84qm2htr8t0pcbvfd1gmrgk3d89937jn38a5a6et9ba967gnj99t4q4mkp6t0kuf8',
    SMART_WALLETS: [
    ...
    RATIO: 0.01, // 跟单资金比例 (0.5表示使用50%的可用资金)
...
class CopyTradeBot {
    constructor() {
        this.tradeLogs = [];
        this.cooldownMap = new Map(); // 代币地址 => 最后交易时间
        this.ws = new ws_1.default('wss://pumpportal.fun/api/data');
        this.setupWebSocket();
    }
    /*********************
     * WebSocket 连接管理
     ...
     * 解析交易信号
     *********************/
    parseTradeSignal(data) {
        try {
            const raw = JSON.parse(data.toString());
            console.log(raw);
            // 验证必要字段
            if (!raw.mint || !raw.amount || !raw.action)
                return null;
            return {
                action: raw.action,
                mint: raw.mint,
                amount: raw.amount * CONFIG.RATIO, // 按比例调整跟单量
                isSolAmount: raw.denominatedInSol,
                timestamp: Date.now()
            };
        }
        catch (err) {
